      ...
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6 md:gap-8 items-stretch">
          ...
          <div className="w-full lg:w-[55%] space-y-3 flex flex-col">
            ...
            const CardContent = <div className={`flex items-start gap-4 p-4 rounded-xl bg-card shadow-soft hover:shadow-card transition-all duration-300 group ${credential.certificateImage ? "cursor-pointer" : ""}`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {credential.certificateImage && <ExternalLink className="w-3 h-3 text-secondary opacity-0 group-hover:opacity-100 transition-opacity" />}
                    </div>
                    ...
                    {credential.certificateImage && <p className="text-xs text-secondary/70 mt-1">
                        Click to view certificate
                      </p>}
                  </div>
                </div>;
            if (credential.certificateImage) {
              return <CertificateDialog key={credential.title} imageSrc={credential.certificateImage} imageAlt={credential.certificateAlt}>
                    {CardContent}
                  </CertificateDialog>;
            }
